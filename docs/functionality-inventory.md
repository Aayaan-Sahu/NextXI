...
- **Coach dashboards panel** — every approved coach, by name, with **Open their dashboard**. That sets a one-hour `nextxi-admin-preview` cookie and lands on `/dashboard/coach` reading as that coach: their approval queue, their new clips, their players, their sessions, and the review screen behind each clip. A bar across the top names whose dashboard it is and carries **Stop**.
  - **Reading only, by construction rather than by a flag.** Every write authorises against the signed-in account, and an admin is not that coach, so sign-off, feedback, a connection request, and a club-invite response would be refused anyway — the preview hides those controls rather than showing buttons that fail. It also skips the one write a coach's own visit performs, marking a clip seen, which would otherwise quietly empty a real coach's new-clips list.
  - The cookie carries no authority: `getAdminPreview` (`lib/admin-preview.ts`) re-checks that the signed-in account is an administrator and that the coach is approved, on every read, so it is inert on any other account and after a demotion.
  - The nav is the coach's, badge included, because that is part of what there is to look at. **Messages are not**: a preview never opens somebody else's conversations, shows their unread count, or subscribes to their realtime channels. Nav links that leave the coach pages (Connections, Messages, Profile) bounce an admin back to `/dashboard/admin`, since those pages redirect admins as they always have.
- Admin **cannot**: re-review already-actioned coaches, manage players/guardians, moderate videos/comments, sign off or comment on a report from a preview, or edit anything else.
