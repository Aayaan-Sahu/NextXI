...
| Styling | React Native `StyleSheet`s over `mobile/lib/theme.ts`, **generated** from `shared/theme.css` — the same file the web imports as its Tailwind v4 theme — with a test that fails when the two drift; `@expo-google-fonts/saira-condensed` + `public-sans` | One design system on web and phone, from one definition. NativeWind was the plan: v5 is the only line that reads the web's Tailwind v4 `@theme`, and its CSS pipeline does not build on SDK 57 (a bare `@import "tailwindcss"` fails in lightningcss); v4 needs Tailwind v3, which would mean a second hand-maintained copy of every token. Revisit when v5 ships stable. |
...
- **Messages** — list: avatar, name, compressed time, "You: …" preview, amber unread pill, local fuzzy search (as `conversation-sidebar.tsx`). Thread: speaker groups (`GROUP_GAP_MS` 5 min), day dividers (`TIME_DIVIDER_GAP_MS` 15 min), receipt under your last own message ("Sending… / Sent / Read 14:02"), composer grows to four lines, 4000-char limit, optimistic send; a failed send keeps its bubble, with "Not delivered · Tap to retry" in place of the receipt (not a draft restore — a bubble that vanishes reads as a lost message). Push when the thread isn't open; badge on the tab. Guardian threads: no composer, footer "Read-only — opening this never changes {firstName}'s unread count."
...
| --- | --- | --- |
| 0 | Bearer auth, `lib/api.ts`, `shared/` extraction, `/api/me`, signup, onboarding, usernames, media sign, videos read/delete, capture columns, `DeviceToken` + push sender, AASA/assetlinks, Expo skeleton (router, generated tokens, fonts, auth screens, EAS dev build) | 3 |
