...
function AnimationPanel({ children }: { children: React.ReactNode }) {
  return (
    <div className="relative flex h-64 w-full items-center justify-center overflow-hidden rounded-[12px] border border-gold-500/40 bg-cream-100">
      {children}
    </div>
  );
}
  ...
  return (
    <AnimationPanel>
      <div className="flex flex-col items-center gap-5">
        <div className="flex size-24 items-center justify-center rounded-[10px] border-2 border-dashed border-rust-500">
          <motion.span
            ...
            className="font-display text-2xl font-bold text-rust-500"
          >
            ↑
          </motion.span>
        </div>
        ...
        <motion.div
          animate={reduced ? { y: -40, opacity: 1 } : { y: [24, -60], opacity: [0, 1, 1, 0] }}
          transition={
            reduced
              ? { duration: 0 }
              : {
        ...
        <div className="h-1 w-36 overflow-hidden rounded-full bg-cream-300">
          <motion.div
            animate={reduced ? { x: "0%" } : { x: ["-100%", "0%"] }}
            transition={
              reduced
                ? { duration: 0 }
                : { duration: 2.4, repeat: Infinity, ease: "easeInOut" }
            }
            className="h-full w-full rounded-full bg-rust-500"
          />
        </div>
      </div>
    </AnimationPanel>
  ...
  return (
    <AnimationPanel>
      <svg viewBox="0 0 220 168" className="h-48 w-auto" fill="none">
        ...
        {LAYER_ONE.map((y1, i) =>
          LAYER_TWO.map((y2, j) => (
            <motion.line
              ...
              strokeWidth={1.5}
              animate={reduced ? { opacity: 0.55 } : { opacity: [0, 0, 1, 0, 0] }}
              transition={
                reduced
                  ? { duration: 0 }
                  : {
        ...
        {LAYER_ONE.map((y, i) => (
          <g key={`in-${y}`}>
            <circle cx={50} cy={y} r={9} className="fill-cream-50 stroke-rust-500" strokeWidth={1.5} />
            <motion.circle
              cx={50}
              cy={y}
              r={9}
              className="fill-rust-500"
              animate={reduced ? { opacity: 1 } : { opacity: [0, 1, 0, 0] }}
              transition={
                reduced
                  ? { duration: 0 }
                  : {
        ...
        {LAYER_TWO.map((y, i) => (
          <g key={`out-${y}`}>
            <circle cx={170} cy={y} r={9} className="fill-cream-50 stroke-gold-500" strokeWidth={1.5} />
            <motion.circle
              cx={170}
              cy={y}
              r={9}
...
export function ConnectAnimation() {
  ...
  return (
    <AnimationPanel>
      <div className="relative w-56 rounded-[10px] border border-gold-500/40 bg-cream-50 p-4">
        <div className="flex items-center gap-3">
          <span className="size-10 rounded-full bg-gold-500" />
          <div className="flex flex-col gap-1.5">
            <span className="h-2 w-24 rounded-full bg-pitch-900/80" />
            <span className="h-2 w-16 rounded-full bg-gold-500/50" />
          </div>
        </div>
        ...
        <div className="relative mt-4">
          <motion.div
            ...
            transition={
              reduced
                ? { duration: 0 }
                : { duration: cycle, times: [0, 0.5, 0.55, 0.95, 1], repeat: Infinity }
            }
            className="rounded-md bg-rust-600 py-2 text-center font-mono text-[11px] font-semibold tracking-[.14em] text-cream-50 uppercase"
          >
            Connect
          </motion.div>
          <motion.div
            ...
            className="absolute inset-0 rounded-md bg-gold-500 py-2 text-center font-mono text-[11px] font-semibold tracking-[.14em] text-pitch-900 uppercase"
          >
            Connected
          </motion.div>
          {reduced ? null : (
            <motion.span
              ...
              className="pointer-events-none absolute top-1/2 left-1/2 size-8 -translate-x-1/2 -translate-y-1/2 rounded-full border-2 border-gold-500"
            />
          )}
        </div>
...
export function StepArrow({ flip = false }: { flip?: boolean }) {
  ...
  return (
    <div className={`flex justify-center py-2 ${flip ? "-scale-x-100" : ""}`} aria-hidden>
      <svg viewBox="0 0 220 130" className="h-28 w-auto text-gold-500" fill="none">
        <motion.path
        ...
        <motion.path
          ...
          viewport={{ once: true, amount: 0.7 }}
          transition={reduced ? { duration: 0 } : { duration: 0.25, delay: 0.85 }}
        />
      </svg>
    </div>
  );
