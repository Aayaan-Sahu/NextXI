...
export function TrustStrip() {
  return (
    <section id="safeguarding" className="scroll-mt-24 bg-pitch-900">
          ...
          {TRUST_ITEMS.map((item) => (
            <div key={item.title}>
              <h3 className="font-display text-xl leading-tight font-semibold text-cream-200 uppercase">
                <Link
                  href={item.href}
                  className="text-cream-200 no-underline hover:text-gold-500 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-gold-500"
                >
                  {item.title}
                </Link>
              </h3>
